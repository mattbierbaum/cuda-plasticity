from SteepestDecentBoundaryPruning import *
...
class ClusteringMixed(SteepestDecentBoundaryPruningMethod):
	def __init__(self,N,vector,boundaries,clusters,preJ=None):
		...
		self.clusters = {}
		for cl in clusters:
			cl.perimeter = numpy.sum([bd.length for bd in cl.boundaries]) 
		...
		for b in self.boundaries:
			boundary_cost.append(self.boundaries[b].deltaInhomogeneity)
			perimeter_cost.append(max([self.boundaries[b].clusterPair[0].perimeterToArea,\
						       self.boundaries[b].clusterPair[1].perimeterToArea])+self.boundaries[b].deltaPerimeter)

		boundary_cost = numpy.array(boundary_cost)
		perimeter_cost = numpy.array(perimeter_cost)
		
		scale_boundary  = boundary_cost.mean()
		scale_perimeter = perimeter_cost.mean()

		self.scale = self.J * scale_perimeter / scale_boundary

		for b in self.boundaries:
			self.costs[b] = (self.boundaries[b].deltaInhomogeneity*self.scale, max([self.boundaries[b].clusterPair[0].perimeterToArea,\
						       self.boundaries[b].clusterPair[1].perimeterToArea]), self.boundaries[b].deltaPerimeter)
	...
	def UpdateClusters(self,c0,c1,b):
		...
		for b in c1.boundaries:
			...
			if dist < c0.radius + cc0.radius + CORESIZE * 2:
				...
				self.costs[b.index] = (self.scale*b.deltaInhomogeneity,max([cc0.perimeterToArea,cc1.perimeterToArea]),b.deltaPerimeter) 
		return c0,c1
	...
	def AddNewBoundary(self,clusterPair,removedBD):
		newb = Boundary(clusterPair,0.,removedBD.length)
		cc0,cc1 = clusterPair
		...
		self.costs[newb.index] = (newb.deltaInhomogeneity*self.scale,max([cc0.perimeterToArea,cc1.perimeterToArea]),newb.deltaPerimeter) 
...
class Initialization:
	...
	def GenerateClustersFromEverySite(self):
		...
		horizontalBD = [Boundary((clusters[i*N+j],clusters[i*N+(j+1)%N]),\
							   sum((self.vector[i,j] -self.vector[i,(j+1)%N])**2), 1.) for i in range(N) for j in range(N)]
		verticalBD = [Boundary((clusters[i*N+j],clusters[((i+1)%N)*N+j]),\
							   sum((self.vector[i,j] -self.vector[i,(j+1)%N])**2), 1.) for i in range(N) for j in range(N)]
...
def Debug(J=0.1):
	...
	dir = "output/"
	N = 128 #16 
	#rodrigues = WallInitializer.TwoRodrigues2DInitializer((N,N))
	#rodrigues = WallInitializer.RandomRodriguesVectorFieldGenerator((N,N),6,False,seed=1)
	#rodrigues = WallInitializer.Smearing2DFields((N,N),rodrigues,20.)
	rod = misc.imread("tests/lena.jpg")#numpy.zeros((N,N,3),float)
	#rod[:,:,0] = rodrigues['x']
	#rod[:,:,1] = rodrigues['y']
	#rod[:,:,2] = rodrigues['z']
	rotate = False
	...
	clustering = ClusteringMixed(N,rod,boundaries,clusters,preJ)
	...
	clustering.GetReconstructionWithBoundary(N,rod,outputDir)#orientationmap,outputDir)
	return rod,clustering

def Run(N,state,J,dir,rod=None,orientationmap=None,snapshot=None,Return=False):
	import PlasticitySystem
	...
	preJ = J#0.5
	system = Initialization(N,rod)
	outputDir = dir+"Inhomo_Clustering_N_"+str(N)+"_preJ="+str(preJ).replace('.','_')
	boundaries,clusters = system.GenerateClustersFromEverySite() 
	...
	if snapshot is not None:
		for i in range(len(snapshot)):
			preJ = snapshot[i]
			outputDir = dir+"Inhomo_Clustering_N_"+str(N)+"_preJ="+str(preJ).replace('.','_')
			clustering = ClusteringOnInhomogeneity(N,rod,clustering.boundaries.values(),clustering.clusters.values(),preJ)
			clustering.Run(outputDir)
			clustering.GetReconstructionWithBoundary(N,orientationmap,outputDir)
	if Return:
		return clustering
...
def RunMixed(N,state,J,dir,rod=None,orientationmap=None,snapshot=None,Return=False):
	import PlasticitySystem
	import FieldInitializer
	import OrientationField
	if (orientationmap is None) or (rod is None):
	...
	preJ = J#0.5
	system = Initialization(N,rod)
	outputDir = dir+"NewClustering_N_"+str(N)+"_preJ="+str(preJ).replace('.','_')
	boundaries,clusters = system.GenerateClustersFromEverySite() 
	clustering = ClusteringMixed(N,rod,boundaries,clusters,preJ)
	clustering.Run(outputDir)
	clustering.GetReconstructionWithBoundary(N,orientationmap,outputDir)
	if snapshot is not None:
		for i in range(len(snapshot)):
			preJ = snapshot[i]
			outputDir = dir+"NewClustering_N_"+str(N)+"_preJ="+str(preJ).replace('.','_')
			clustering = ClusteringMixed(N,rod,clustering.boundaries.values(),clustering.clusters.values(),preJ)
			clustering.Run(outputDir)
			clustering.GetReconstructionWithBoundary(N,orientationmap,outputDir)
	if Return:
		return clustering

def RunSeperated(N,state,J,PtoA,dir,rod=None,orientationmap=None,snapshot=None,statefile=None,Return=False):
