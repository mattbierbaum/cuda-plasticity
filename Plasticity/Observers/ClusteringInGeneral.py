...
class ClusteringMixed(SteepestDecentBoundaryPruningMethod):
    """
    Run boundary-pruning method based on both inhomogeneity and PtoA 
    from the full lattice grid.
    """
...
class ClusteringOnInhomogeneity(SteepestDecentBoundaryPruningMethod):
    """
    Run boundary-pruning method only based on inhomogeneity from the full lattice grid.
    """
    def __init__(self,N,vector,boundaries,clusters,preJ=None):
        self.N = N
        self.vector = vector
...
class ClusteringOnPerimeterToArea(SteepestDecentBoundaryPruningMethod):
    """
    Run boundary-pruning method only based on PtoA from some intermediate state.
    """
    def __init__(self,N,boundaries,clusters,preJ=None):
        self.N = N
        self.clusters = clusters
        for c in self.clusters.values():
            c.perimeter = c.GetPerimeter()
            c.perimeterToArea = c.perimeter/len(c.sites)
        self.boundaries = boundaries
        for  bd in self.boundaries.values():
            c0,c1 = bd.clusterPair
            bd.deltaPerimeter = bd.GetDeltaPerimeterOfCombinedCluster()
            bd.length = bd.deltaPerimeter/2.
...
class PostClustering(SteepestDecentBoundaryPruningMethod):
    """
    Run boundary-pruning method based on both PtoA and inhomogeneity from some intermediate state.
    """
    def __init__(self,N,vector,boundaries,clusters,preJ=None):
        self.N = N
        self.vector = vector
...
def Debug(J=0.1):
    ...
    rodrigues = WallInitializer.RandomRodriguesVectorFieldGenerator((N,N),4,False,seed=1)
    ...
    outputDir = dir+"DDDOldClustering_Core_"+str(CORESIZE)+"_N_"+str(N)+"_preJ="+str(preJ).replace('.','_')
...
def Run(N,state,J,dir,rod=None,orientationmap=None,snapshot=None,Return=False):
    ...
    preJ = J#0.5
    system = Initialization(N,rod)
    outputDir = dir+"Inhomo_Clustering_Core_"+str(CORESIZE)+"_N_"+str(N)+"_preJ="+str(preJ).replace('.','_')
    boundaries,clusters = system.GenerateClustersFromEverySite() 
    clustering = ClusteringOnInhomogeneity(N,rod,boundaries,clusters,preJ)
    clustering.Run(outputDir)
    clustering.GetReconstructionWithBoundary(N,orientationmap,outputDir)
...
def RunMixed(N,state,J,dir,rod=None,orientationmap=None,snapshot=None,Return=False):
    ...
    preJ = J#0.5
    system = Initialization(N,rod)
    outputDir = dir+"NewClustering_Core_"+str(CORESIZE)+"_N_"+str(N)+"_preJ="+str(preJ).replace('.','_')
    boundaries,clusters = system.GenerateClustersFromEverySite() 
...
def RunSeperated(N,state,J,PtoA,dir,rod=None,orientationmap=None,snapshot=None,Return=False):
    import PlasticitySystem
    import FieldInitializer
    import OrientationField
    if (orientationmap is None) or (rod is None):
    ...
    preJ = J
    system = Initialization(N,rod)
    outputDir = dir+"_Inhomo_Clustering_Core_"+str(CORESIZE)+"_N_"+str(N)+"_preJ="+str(preJ).replace('.','_')
    boundaries,clusters = system.GenerateClustersFromEverySite() 
    clustering = ClusteringOnInhomogeneity(N,rod,boundaries,clusters,preJ)
    clustering.Run(outputDir)
    clustering.GetReconstructionWithBoundary(N,orientationmap,outputDir)
    outputDir = dir+"_Post_Inhomo_Clustering_Core_"+str(CORESIZE)+"_N_"+str(N)+"_preJ="+str(preJ).replace('.','_')+\
