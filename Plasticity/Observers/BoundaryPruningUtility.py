...
"""
core_neighbors = [(i,j) for i in range(-CORESIZE*2,CORESIZE*2+1) \
                   for j in range(-CORESIZE*2,CORESIZE*2+1) \
                   if (i**2+j**2)<=(CORESIZE*2)**2 and (i != 0 or j != 0)
                       and ((i>0 and j>=0) or (i<=0 and j>0))]
core_neighbors_redundant = [(i,j) for i in range(-CORESIZE*2,CORESIZE*2+1) \
                   for j in range(-CORESIZE*2,CORESIZE*2+1) \
                   if (i**2+j**2)<=(CORESIZE+2)**2 and (i != 0 or j != 0)]
# It is not necessary to normalize them.
kernel = lambda dx,dy: numpy.exp(-((dx)**2+(dy)**2)/2./CORESIZE**2)
kernel2 = lambda dxy2: numpy.exp(-(dxy2)/2./CORESIZE**2)*(dxy2<=(CORESIZE+2)**2)
                ...
                for(int i=0; i<sz1; i++) {
                    ...
                    for(int j=0; j<sz2; j++) {
                        ...
                        if (dxy2 <= square(%(coresize)d+2)) {
                            factor +=2.*exp(-(double)dxy2/2./square(%(coresize)d));
