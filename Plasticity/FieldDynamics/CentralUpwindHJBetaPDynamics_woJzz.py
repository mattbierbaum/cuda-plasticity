...
class BetaPDynamics(CentralUpwindHJDynamics):
    ...
    def BetaP2D_Velocity(self,u,ux,uy,opt=None):
        ...
        if self.Lambda != 0:
            #glide only
            sigma_tr = (sigma[x,x]+sigma[y,y]+sigma[z,z])/2.*self.Lambda
            velocity[x] += sigma_tr*(ux[y,y]-uy[x,y])
            velocity[y] += sigma_tr*(uy[x,x]-ux[y,x])
            velocity[z] -= sigma_tr*(ux[z,x]+uy[z,y])
    ...
    def H_2D(self,field,deriv_x,deriv_y,opt=None):
        ...
        # glide only
        if self.Lambda != 0:
            J_trace = self.Lambda/2.*(V[x]*(deriv_x[y,y]-deriv_y[x,y])+V[y]*(deriv_y[x,x]-deriv_x[y,x])-V[z]*(deriv_x[z,x]+deriv_y[z,y]))
            for i in [x,y]:
                H[i,i] -= J_trace 
        return H
    ...
    def CalculateFlux(self, time, state, CFLCondition=False):
        ...
        if CFLCondition:
            rhs,v = CentralUpwindHJDynamics.CalculateFlux(self,time,state,CFLCondition=CFLCondition) 
            rhs[z,z] = 0.
            return rhs,v
        else:
            rhs = CentralUpwindHJDynamics.CalculateFlux(self,time,state,CFLCondition=CFLCondition) 
            rhs[z,z] = 0.
            return rhs
