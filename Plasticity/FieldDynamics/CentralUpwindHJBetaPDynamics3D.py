from Plasticity.FieldDynamics.CentralUpwindHJ3D import *
...
class BetaPDynamics(CentralUpwindHJDynamics):
    ...
    def BetaP3D_Velocity(self,u,ux,uy,uz,opt=None):
        ...
        if self.Lambda != 0:
            ...
            velocity[x] += sigma_tr*(ux[x,x]+ux[y,y]+ux[z,z])
            velocity[y] += sigma_tr*(uy[x,x]+uy[y,y]+uy[z,z])
            velocity[z] += sigma_tr*(uz[x,x]+uz[y,y]+uz[z,z])
            for l in velocity.components:
                velocity[l] -= sigma_tr*(ux[l,x]+uy[l,y]+uz[l,z])

        velocity /= self.BetaP3D_RhoMod(u,ux,uy,uz)
    ...
    def H_2D(self,field,deriv_x,deriv_y,opt=None):
        ...
        # glide only
        if self.Lambda != 0:
            J_trace = -self.Lambda/3.*(V[x]*(deriv_x[y,y]+deriv_x[z,z]-deriv_y[x,y])+\
                                       V[y]*(deriv_y[x,x]+deriv_y[z,z]-deriv_x[y,x])-\
                                       V[z]*(deriv_x[z,x]+deriv_y[z,y]))
            for i in [x,y,z]:
                H[i,i] += J_trace
        return H
    ...
    def H_3D(self,field,deriv_x,deriv_y,deriv_z,opt=None):
        ...
        # glide only
        if self.Lambda != 0:
            J_trace = -self.Lambda/3.*(V[x]*(deriv_x[y,y]+deriv_x[z,z]-deriv_y[x,y]-deriv_z[x,z])+\
                                       V[y]*(deriv_y[x,x]+deriv_y[z,z]-deriv_x[y,x]-deriv_z[y,z])+\
                                       V[z]*(deriv_z[x,x]+deriv_z[y,y]-deriv_x[z,x]-deriv_y[z,y]))
            for i in [x,y,z]:
                H[i,i] += J_trace
        return H
